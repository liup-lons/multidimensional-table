...
### 视图类型
- **表格视图**: 支持复杂数据结构的表格展示，斑马纹行背景，表头固定，支持公式字段
- **看板视图**: 按字段分组展示卡片，支持拖拽排序，卡片hover效果
- **日历视图**: 日/周/月视图切换，事件卡片展示，日期选中高亮
- **甘特视图**: 时间轴展示，甘特条拖拽，时间范围缩放，任务进度可视化
- **数据仪表盘**: 集成ECharts图表，统计卡片、柱状图、饼图、折线图
- **自动化工作流**: 可视化工作流配置，支持多种触发条件和执行动作
...
### 公式计算引擎
支持60+内置函数：
- **数学函数**: SUM、AVG、MIN、MAX、COUNT、ROUND、ABS、SQRT等
- **文本函数**: CONCAT、LEFT、RIGHT、MID、LEN、UPPER、LOWER、TRIM等
- **日期函数**: TODAY、NOW、DATE、YEAR、MONTH、DAY、DATEDIF、EDATE等
- **逻辑函数**: IF、AND、OR、NOT、IFERROR、SWITCH等
- **查找函数**: INDEX、MATCH等
- **字段引用**: 使用 `[字段名]` 语法引用其他字段值

### 自动化工作流
- **触发方式**: 记录创建/更新/删除、字段变更、定时执行（Cron表达式）
- **执行动作**: 更新字段、发送通知、调用Webhook、发送邮件
- **条件判断**: 支持多条件组合判断
- **执行日志**: 完整的工作流执行记录和日志追踪
